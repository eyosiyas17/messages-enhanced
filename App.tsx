...
const getCustomizedName = (phone: string, fallbackName?: string): string => {
  const normalized = normalizePhone(phone);
  const custom = PHONE_CUSTOMIZATIONS[normalized];
  return custom?.name || fallbackName || phone;
};

const getCustomizedColor = (phone: string, fallbackColor?: string): string => {
  const normalized = normalizePhone(phone);
  const custom = PHONE_CUSTOMIZATIONS[normalized];
  return custom?.color || fallbackColor || pickPaletteColor(phone);
};

const getCustomizedAvatar = (phone: string, fallbackAvatar?: string): string => {
  const normalized = normalizePhone(phone);
  const custom = PHONE_CUSTOMIZATIONS[normalized];
  return custom?.avatar || fallbackAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(phone)}&backgroundColor=${encodeURIComponent(toDicebearColor(pickPaletteColor(phone)))}`;
};
  ...
  const handleImportOldMessages = useCallback(async (messages: DeviceSMS[]) => {
    ...
    try {
      ...
      for (const phoneKey of Object.keys(grouped)) {
        ...
        // Use customization system for name, color, and avatar
        const customizedName = getCustomizedName(phoneKey, contactMatch?.name || existing?.name);
        const customizedColor = getCustomizedColor(phoneKey, contactMatch?.color);
        const customizedAvatar = getCustomizedAvatar(phoneKey, existing?.avatar);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const oldMessages = await fetchOldMessages();
        const currentMessageCount = oldMessages.length;
        
        // Only process if we have new messages or if it's been a while
        const hasNewMessages = currentMessageCount > lastMessageCount;
        const shouldProcess = hasNewMessages || (Date.now() - lastProcessedTimestamp > 30000);
        
        if (shouldProcess && oldMessages.length > 0) {
          console.log('SMS polling: Processing', oldMessages.length, 'messages (new:', hasNewMessages, ')');
          await handleImportOldMessages(oldMessages);
          lastMessageCount = currentMessageCount;
          lastProcessedTimestamp = Date.now();
          console.log('SMS polling: Successfully processed messages');
        } else {
          console.log('SMS polling: No new messages to process');
        }
      } catch (e) {
